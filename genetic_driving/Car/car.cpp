...
float Car::distWall(float angle, const sf::Image& img) {
    sf::Vector2f direction = normalizeVector2f(vectAngle(angleVect(orientation) + angle));
    float distance = 0.0f;
    const float increment = 5.0f;
    const int img_x = img.getSize().x;
    const int img_y = img.getSize().y;
    ...
    while (distance < viewDist) {
        ...
        testPoint.x = std::clamp(testPoint.x, 0.0f, static_cast<float>(img_x - 1));
        testPoint.y = std::clamp(testPoint.y, 0.0f, static_cast<float>(img_y - 1));
...
void Car::update(sf::Image& img, std::vector<Checkpoint>& checkpoints, std::vector<Car>& BestCars) {
    ...
    if (!dead) {
        ...
        float speedC = minSpeed + (abs(nnOutputs[0]) / 2);
